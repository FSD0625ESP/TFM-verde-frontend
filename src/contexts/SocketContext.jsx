...
export const SocketProvider = ({ children }) => {
    ...
    const onDeliveryUpdate = useCallback((callback) => {
        if (socket) {
    ...
    const startOrderShipping = useCallback((orderId) => {
        if (socket && isConnected && orderId) {
    ...
    const value = useMemo(() => ({
        ...
        unreadCount,
        setUnreadCount,
    }), [socket, isConnected, joinChats, sendMessage, emitTyping, emitStopTyping, markAsRead, onNewMessage, onUserTyping, onUserStopTyping, onMessagesRead, requestPresence, onPresenceStatus, onUserOnline, onUserOffline, joinDelivery, onDeliveryUpdate, joinOrder, startOrderShipping, onOrderUpdate, unreadCount, setUnreadCount]);
  ...
  // para informar de nuevos pedidos al vendedor
