...
export default function Filtros({ categoriesList, storesList }) {
  const [productsList, setProductsList] = useState([]);
  const [selectedCategories, setSelectedCategories] = useState([]);
  const [selectedStores, setSelectedStores] = useState(
    storesList.length === 1 ? [storesList[0]._id] : []
  );
  ...
  // referencia al AbortController actual para cancelar peticiones que ya no interesan
  ...
  const loadMore = useCallback(async () => {
    ...
    setLoading(true);
    console.log("Cargando más productos, página:", page);
  ...
  */
  ...
  const handleStoreToggle = (store) => {
    ...
    console.log("handleStoreToggle - selectedStores", selectedStores);
  };
  ...
  const handleCategoryToggle = (category) => {
    ...
    console.log(
      "handleCategoryToggle - selectedCategories",
      selectedCategories
    );
  };
  ...
  const handleFilters = async () => {
    ...
    if (resultsContainerRef.current) {
      ...
      await waitForScroll(resultsContainerRef);
    }
        ...
        <aside className="sticky top-[100px] self-start col-span-12 sm:col-span-6 md:col-span-4 lg:col-span-3 flex flex-col justify-start bg-primary/40 p-4 rounded-lg">
          ...
          {storesList && storesList.length > 1 && (
            <div className="flex flex-col gap-2 mt-4">
              <h2 className="text-xl font-semibold">Filtrar por tienda</h2>
          ...
          <div className="flex flex-col gap-2 mt-4">
            <h2 className="text-xl font-semibold">Filtrar por oferta</h2>
            <div className="flex flex-col flex-wrap gap-2">
              <Button
                className={`bg-primary/30 hover:bg-secondary/40 rounded-lg px-4 py-2 ${
