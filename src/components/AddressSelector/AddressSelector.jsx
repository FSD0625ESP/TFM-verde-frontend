...
import { Card, CardBody, CardHeader, Button, Divider, Spinner, useDisclosure, addToast } from "@heroui/react";
import { MapPin, Edit, Trash2, Plus, ChevronDown, Check } from "lucide-react";
import { getUserAddresses, setDefaultAddress, deleteAddress } from "../../services/api";
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl sm:text-2xl font-bold text-gray-900">
                                {mode === "checkout" ? "Selecciona una dirección" : "Mis Direcciones"}
                            </h2>
                ...
                {loadingAddresses ? (
                    ...
                    <div className="grid grid-cols-1 gap-4 auto-rows-auto">
                        ...
                        {addresses.map((address) => (
                            <Card
                                key={address._id}
                                className={`shadow-sm hover:shadow-md transition-shadow cursor-pointer ${address.isDefault && mode === "view" ? "border-2 border-teal-600" : ""
                                    } ${mode === "checkout" && selected === address._id ? "border-2 border-teal-600 bg-teal-50" : ""
                                    }`}
                                onClick={() => mode === "checkout" && handleSelectAddress(address._id)}
                            >
                                <CardHeader className="flex justify-between items-start p-4">
                                    ...
                                    {mode === "view" && (
                                        <button
                                            onClick={() =>
                                                setExpandedAddressId(expandedAddressId === address._id ? null : address._id)
                                            }
                                            className="text-gray-600 hover:text-gray-900 ml-2 shrink-0"
                                        >
                                            <ChevronDown
                                                size={20}
                                                className={`transition-transform ${expandedAddressId === address._id ? "rotate-180" : ""}`}
                                            />
                                        </button>
                                    )}
                                </CardHeader>

                                {(mode === "checkout" || expandedAddressId === address._id) && (
                                    <>
                                        <Divider />
                                        <CardBody className="space-y-3 text-sm p-4">
                                            <div>
                                                <p className="font-semibold text-gray-700">Dirección</p>
                                                <p className="text-gray-600 wrap-break-word">{address.street}</p>
                                            </div>
